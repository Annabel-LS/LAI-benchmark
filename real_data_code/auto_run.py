...
================================
功能：
  1. 遍历 chr1 ~ chr22 子文件夹，从每个文件夹的 ASW.phased 中提取个体 NA00000，
     生成对应 chrN 文件夹下的 NA00000_chrN.phased
  2. 调用 to_vcf.py 将该个体文件与 CHS/CEU/YRI 三个参考文件转换为 VCF，
     输出到同一 chrN 文件夹下
  3. 用 bgzip 压缩生成的 VCF 文件

支持断点续传：若对应染色体的 NA0000_chrN.phased、NA00000_chrN.vcf.gz、ref_chrN.vcf.gz 均已存在，
               则跳过该染色体（除非使用 --force）。
...
def extract_individual_phased(asw_file: str, ind_id: str, out_file: str) -> None:
    ...
    参数：
        ...
        ind_id   : 个体名（如 NA00000），函数会自动查找 ind_id_A 和 ind_id_B 列
...
def main():
    ...
    for chrom in range(1, 23):
        ...
        extracted_phased = os.path.join(chr_dir, f"NA0000_chr{chrom}.phased")
        out_vcf_sample = os.path.join(chr_dir, f"NA0000_chr{chrom}.vcf")
        ...
        # 1. 提取 NA00000（如果已存在，可跳过？但为了保险，重新提取）
        try:
            extract_individual_phased(asw_file, "NA00000", extracted_phased)
        except Exception:
